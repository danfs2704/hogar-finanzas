            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="p-4 rounded-xl bg-rose-50">
                  ...
                  <p className="text-xl font-bold text-rose-700 mt-1">{formatCurrencyARS(totalMemberExpense)}</p>
                </div>
                <div className="p-4 rounded-xl bg-emerald-50">
                  <p className="text-xs text-emerald-600 font-medium">Ingreso Total</p>
                  <p className="text-xl font-bold text-emerald-700 mt-1">{formatCurrencyARS(totalMemberIncome)}</p>
                </div>
                <div className={`p-4 rounded-xl ${totalMemberIncome - totalMemberExpense >= 0 ? 'bg-blue-50' : 'bg-amber-50'}`}>
                  ...
                  <p className={`text-xl font-bold mt-1 ${totalMemberIncome - totalMemberExpense >= 0 ? 'text-blue-700' : 'text-amber-700'}`}>{formatCurrencyARS(totalMemberIncome - totalMemberExpense)}</p>
                </div>
              </div>
                  ...
                  <div key={m.memberId}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-semibold text-rose-600">{formatCurrencyARS(m.totalExpense)}</p>
                    ...
                    {m.totalIncome > 0 && (
                      <p className="text-xs text-emerald-600 mt-1">Ingresos: {formatCurrencyARS(m.totalIncome)}</p>
                    )}
                  </div>
                ))}
              </div>
                    ...
                    <div className="ml-4 space-y-1.5">
                      {cat.subcategories.map(sub => (
                        <div key={sub.subcategoryId} className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-600 font-medium">{formatCurrencyARS(sub.total)}</span>
                ...
                <div key={i} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-600 w-24 text-right">{formatCurrencyARS(m.income)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-600 w-24 text-right">{formatCurrencyARS(m.expense)}</span>
                    </div>
                  </div>
                </div>
              );
              ...
              <div key={acc.accountId} className="p-4 rounded-xl border border-slate-100">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-slate-800">{acc.currency === 'USD' ? formatCurrencyUSD(acc.balance) : formatCurrencyARS(acc.balance)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-emerald-500">Ingresos</span>
                    <span className="text-emerald-600">+{acc.currency === 'USD' ? formatCurrencyUSD(acc.income) : formatCurrencyARS(acc.income)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-rose-500">Gastos</span>
                    <span className="text-rose-600">-{acc.currency === 'USD' ? formatCurrencyUSD(acc.expense) : formatCurrencyARS(acc.expense)}</span>
                  </div>
                </div>
              </div>
            ))}
